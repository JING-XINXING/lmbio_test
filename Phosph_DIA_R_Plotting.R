########################################################################################################
# /Rscript function: Phosph_DIA_project R analysis and plotting/
# /Author: jingxinxing, geshuting, tianquan
# /Date: 2020/03/31
# /Version: v1.4
# /New: 1.本脚本专门用于磷酸化DIA项目数据分析和可视化绘图；
...
## 3.读取数据及数据处理
### 3.3 磷酸化的蛋白、肽段、位点统计图（读取数据：ppp.txt）
...
# bp <- barplot(N, width = 3, col = c("darkturquoise", "lightgreen", "lightcoral"), names.arg = c("Proteins", "Peptides", "Sites"), legend = c("PhosphoProteins", "PhosphoPeptides", "PhosphoSites"), xlab = NULL, ylab = c("Number"), ylim = c(0, ymax + 2000), axes = T, xpd = F, axisnames = TRUE, horiz = F, space= 1.5, beside = TRUE, cex.names = 1.8, cex.axis = 1.1, cex.lab = 1.8, args.legend = list(x = "top"))
...
N2 <- N + 2000
text(bp, N2, format(N), xpd = TRUE, col = "black", cex = 2)
par(op)
dev.off()

#### 保存PNG图片
png("3.Phosphorylated_protein-peptide-site_statistics_barplot.png", width = 1000, height = 1000)
par(mai=c(2,1.5,0.8,1.5))
# bp <- barplot(N, width = 3, col = c("darkturquoise", "lightgreen", "lightcoral"), names.arg = c("Proteins", "Peptides", "Sites"), legend = c("PhosphoProteins", "PhosphoPeptides", "PhosphoSites"), xlab = NULL, ylab = c("Number"), ylim = c(0, ymax + 2000), axes = T, xpd = F, axisnames = TRUE, horiz = F, space= 1.5, beside = TRUE, cex.names = 1.8, cex.axis = 1.1, cex.lab = 1.8, args.legend = list(x = "top"))
bp <- barplot(N, width = 3, col = c("blue", "orangered", "darkgreen"), names.arg = c("Proteins", "Peptides", "Sites"), legend = c("Phospho Proteins", "Phospho Peptides", "Phospho Sites"), xlab = NULL, ylab = c("Number"), ylim = c(0, ymax + ymax/3), axes = T, xpd = F, axisnames = TRUE, horiz = F, space= 1.5, beside = TRUE, cex.names = 1.5, cex.axis = 1.5, cex.lab = 1.5, args.legend = list(x = "top"))
...
pdf("4.Quantitative_distribution_of_phosphorylated_peptide_sites_pie.pdf", width = 15, height = 8.5)
pie(x, labels = labels, lwd = 15, col = c("lightgreen", "tomato", "cornflowerblue", "orange"), border = FALSE)
dev.off()

png("4.Quantitative_distribution_of_phosphorylated_peptide_sites_pie.png", width = 1500, height = 850)
pie(x, labels = labels, lwd = 15, col = c("lightgreen", "tomato", "cornflowerblue", "orange"), border = FALSE)
dev.off()
...
### 3.5 STY位点分布图
sty_data <- read.table("./stats_data/STY.txt", sep = "\t", quote = "\"'")
print(sty_data)
...
SNP_Lab <- paste("S site", paste("Percentage:", sprintf("%.2f",SNP*100), "%"))
SNP_Lab

TNP_Lab <- paste("T site", paste("Percentage:", sprintf("%.2f",TNP*100), "%"))
TNP_Lab

YNP_Lab <- paste("Y site", paste("Percentage:", sprintf("%.2f",YNP*100), "%"))
YNP_Lab

labels <- c(SNP_Lab, TNP_Lab, YNP_Lab)

#### 绘制2D饼图
pdf("5.STY_locus_distribution_pie.pdf", width = 12, height = 8.5)
pie(x, labels = labels, col = c("tomato", "turquoise", "gold"), main = "STY sites distribution", border = FALSE)
dev.off()

png("5.STY_locus_distribution_pie.png", width = 1200, height = 850)
pie(x, labels = labels, col = c("tomato", "turquoise", "gold"), main = "STY sites distribution", border = FALSE)
dev.off()
...
#   class(data)
#   rownames(data)
#   #### 3.4.2 新建一个Abundance_distribution_map目录
#   dir()
...
# 
#     pdf(paste0(i,".Phosphorylated_Peptides_Abundance_distribution_",new_compa_groups,".pdf"), width = 12, height = 7.5)
#     # hist(data_new[,2],col="greenyellow",freq=FALSE,breaks=30,
#     hist(data_new[,2],col="limegreen",freq=FALSE,breaks=30,
#          main="Phosphorylated Peptides Ratio Distribution",
#          xlab="log2(FC)",ylab="Number of Phosphorylated Peptides")
#     dev.off()
#     png(paste0(i,".Phosphorylated_Peptides_Abundance_distribution_",new_compa_groups,".png"), width = 1200, height = 750)
#     hist(data_new[,2],col="limegreen",freq=FALSE,breaks=30,
#          main="Phosphorylated Peptides Ratio Distribution",
#          xlab="log2(FC)",ylab="Number of Phosphorylated Peptides")
#     dev.off()
#     write.csv(data, file = paste0(i,".Phosphorylated_Peptides_Abundance_distribution_",new_compa_groups,"_data.csv"))
#     dir()
#   }
#   setwd("../")
#   dir()
...
print("输出R包和环境变量信息：")
sessionInfo()
print("磷酸化项目R绘图已经完成，请下载本地查看图片数据和文件")
print("祝工作顺利，开心每一天！")
