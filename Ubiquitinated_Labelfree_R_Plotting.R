########################################################################################################
# /Rscript function: Ubiquitinated_Labelfree_project R analysis and plotting/
...
#        英文：1.Statistical_histogram_of_ubiquitinated_protein_group.p*

#        中文：2.乙酰化蛋白分子质量分布图
#        英文：2.Molecular_weight_distribution_of_ubiquitinated_protein.p*

#        中文：3.乙酰化蛋白位点数量分布图
#        英文：3. Quantitative_distribution_of_ubiquitinated_protein_sites.p*

#        中文：4.乙酰化肽段位点数量分布图
#        英文：4.Quantitative_distribution_of_ubiquitinated_peptide_sites.p*
...
## 1.设置当前工作目录
# setwd("D:/工作空间/鹿明生物蛋白质组学信息分析/研发/2019年研发工作目录/R_projects/ubiquitinated_Project_R_plotting_v1/")
setwd("./")
dir()
...
ggplot(mw_data, aes(mw_data$Mol..weight..kDa.)) +
  ...
  # geom_histogram(binwidth = 5, fill = "brown1", aes(y=..density..)) +
...
ggsave("1.Molecular_weight_distribution_of_ubiquitinated_protein.pdf", width = 12, height = 7.5, units = "in")
ggsave("1.Molecular_weight_distribution_of_ubiquitinated_protein.png", width = 12, height = 7.5, units = "in")
...
ggplot(nump_data, aes(x = Counts, y = Freq)) +
  ...
  xlab(label = "Number of Ubiquitinated Sites in a Protein") +
  ylab(label = "Ubiquitinated Protein Numbers") +
  theme_bw() +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
  geom_text(aes(label = Freq, vjust = -0.8, hjust = 0.5), size = 3) +
  theme(plot.margin = unit(rep(2,4),'lines'), aspect.ratio = 0.5) +
  theme(axis.title.x = element_text(size=16), axis.title.y=element_text(size=16))
ggsave("2.Quantitative_distribution_of_ubiquitinated_protein_sites.pdf", width = 12, height = 7.5, units = "in")
ggsave("2.Quantitative_distribution_of_ubiquitinated_protein_sites.png", width = 12, height = 7.5, units = "in")
...
pdf("3.Ubiquitinated_protein-peptide-site_statistics_barplot.pdf", width = 10, height = 10)
par(mai=c(2,1.5,0.8,1.5))
# bp <- barplot(N, width = 3, col = c("darkturquoise", "lightgreen", "lightcoral"), names.arg = c("Proteins", "Peptides", "Sites"), legend = c("Ubiquitinated Proteins", "Ubiquitinated Peptides", "Ubiquitinated Sites"), xlab = NULL, ylab = c("Number"), ylim = c(0, ymax + 2000), axes = T, xpd = F, axisnames = TRUE, horiz = F, space= 1.5, beside = TRUE, cex.names = 1.8, cex.axis = 1.1, cex.lab = 1.8, args.legend = list(x = "top"))
bp <- barplot(N, width = 3, col = c("blue", "orangered", "darkgreen"), names.arg = c("Proteins", "Peptides", "Sites"), legend = c("Ubiquitinated Proteins", "Ubiquitinated Peptides", "Ubiquitinated Sites"), xlab = NULL, ylab = c("Number"), ylim = c(0, ymax + 500), axes = T, xpd = F, axisnames = TRUE, horiz = F, space= 1.5, beside = TRUE, cex.names = 1.8, cex.axis = 1.1, cex.lab = 1.8, args.legend = list(x = "top"))
N2 <- N + ymax/10.0
...
#### 保存PNG图片
png("3.Ubiquitinated_protein-peptide-site_statistics_barplot.png", width = 1000, height = 1000)
par(mai=c(2,1.5,0.8,1.5))
# bp <- barplot(N, width = 3, col = c("darkturquoise", "lightgreen", "lightcoral"), names.arg = c("Proteins", "Peptides", "Sites"), legend = c("UbiquitinatedProteins", "UbiquitinatedPeptides", "UbiquitinatedSites"), xlab = NULL, ylab = c("Number"), ylim = c(0, ymax + 2000), axes = T, xpd = F, axisnames = TRUE, horiz = F, space= 1.5, beside = TRUE, cex.names = 1.8, cex.axis = 1.1, cex.lab = 1.8, args.legend = list(x = "top"))
bp <- barplot(N, width = 3, col = c("blue", "orangered", "darkgreen"), names.arg = c("Proteins", "Peptides", "Sites"), legend = c("Ubiquitinated Proteins", "Ubiquitinated Peptides", "Ubiquitinated Sites"), xlab = NULL, ylab = c("Number"), ylim = c(0, ymax + 500), axes = T, xpd = F, axisnames = TRUE, horiz = F, space= 1.5, beside = TRUE, cex.names = 1.8, cex.axis = 1.1, cex.lab = 1.8, args.legend = list(x = "top"))
N2 <- N + ymax/10.0
text(bp, N2, format(N), xpd = TRUE, col = "blue", cex = 2)
par(op)
...
#### 设置饼图的标记labels
labels <- c(paste("Ubiquitinated sites 1, Peptides_number", A_Lab_OK), paste("Ubiquitinated sites 2, Peptides_number", B_Lab_OK))

pdf("4.Quantitative_distribution_of_ubiquitinated_peptide_sites_pie.pdf", width = 15, height = 8.5)
pie(x, labels = labels, lwd = 15, col = c("orangered", "lightseagreen", "cornflowerblue", "brown"), border = FALSE)
dev.off()

png("4.Quantitative_distribution_of_ubiquitinated_peptide_sites_pie.png", width = 1500, height = 850)
...
### Statistics目录内文件检查
if (length(list.files("./Statistics")) == 8) {
    print("目录Statistics内的图片文件已经生成！")
} else if (length(list.files("./Statistics")) == 9) {
    file.remove("./Statistics/Rplots.pdf")
}
...
#   sample_info_1 <- sample_info_1[-which(sample_info_1 == "mix"),]
# } else {
#   print("sample_information.xlsx文件中没有混合样本：MIX、Mix、mix，所以不做删除处理！")
#   print(sample_info_1)
# }
# 
# class(sample_info_1)
...
# xmax <- max(data$`log2(FC)`)
# xmin <- min(data$`log2(FC)`)
#     pdf(paste0(i,".Ubiquitinated_Sites_Abundance_distribution_",new_compa_groups,".pdf"), width = 12, height = 7.5)
#     # hist(data_new[,2],col="greenyellow",freq=FALSE,breaks=30,
#     hist(data_new[,2],col="limegreen",freq=FALSE,breaks=30,
# #         main="Ubiquitinated Sites Ratio Distribution",
#          xlab="log2(FC)",ylab="Number of Ubiquitinated Sites",
# 	 xlim = c(xmin-2,xmax+2), ylim = c(0, 0.6)
# 	)
#     dev.off()
#     png(paste0(i,".Ubiquitinated_Sites_Abundance_distribution_",new_compa_groups,".png"), width = 1200, height = 750)
#     hist(data_new[,2],col="limegreen",freq=FALSE,breaks=30,
# #         main="Ubiquitinated Sites Ratio Distribution",
#          xlab="log2(FC)",ylab="Number of Ubiquitinated Sites",
# 	 xlim = c(xmin-2,xmax+2), ylim = c(0, 0.6)
# 	)
#     dev.off()
#     write.csv(data, file = paste0(i,".Ubiquitinated_Sites_Abundance_distribution_",new_compa_groups,"_data.csv"))
#     dir()
#   }
#   setwd("../")
#   dir()
